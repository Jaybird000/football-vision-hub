...
export function NaariShakti() {
  const { t } = useLang();
  return (
    <section className="on-dark relative py-24 md:py-32 overflow-hidden bg-pitch-black">
      <div className="absolute inset-0 z-0">
        <img src={naariImg} alt="" loading="lazy" width={1400} height={1000} className="w-full h-full object-cover opacity-80" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="max-w-xl bg-pitch-black/70 backdrop-blur-sm p-10 md:p-12 border-l-4 border-neon-strike">
          <span className="text-gold-ink font-bold text-xs uppercase tracking-[0.25em]">Spotlight</span>
          ...
          <button className="mt-8 px-8 py-4 bg-neon-strike text-ink font-display text-xl uppercase tracking-wide hover:scale-[1.03] transition-transform">
            {t("home", "naariCta")}
          </button>
        </div>
      </div>
