...
export function PageHero({ eyebrow, title, sub, image, quote }: { eyebrow: string; title: ReactNode; sub: string; image: string; quote?: Quote }) {
  return (
    <section className="relative min-h-[78vh] flex items-end overflow-hidden border-b border-chalk/10">
      <div className="absolute inset-0 z-0">
        <img src={image} alt="" width={1600} height={900} className="w-full h-full object-cover opacity-50 animate-ken-burns" />
        <div className="absolute inset-0 bg-gradient-to-t from-pitch-black via-pitch-black/50 to-pitch-black/40" />
      </div>
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-24 w-full">
        <span className="text-neon-strike font-bold text-xs uppercase tracking-[0.3em] animate-fade-up">{eyebrow}</span>
        ...
        {quote ? (
          <figure className="mt-10 max-w-3xl border-l-4 border-neon-strike pl-6 animate-fade-up" style={{ animationDelay: "0.4s" }}>
            ...
            <figcaption className="mt-4 text-xs uppercase tracking-[0.25em] text-chalk/60">
              — {quote.author}{quote.role ? <span className="text-chalk/40"> · {quote.role}</span> : null}
            </figcaption>
          </figure>
        ) : (
        ...
        {quote && (
          <p className="mt-8 max-w-2xl text-base text-chalk/65 leading-relaxed animate-fade-up" style={{ animationDelay: "0.55s" }}>{sub}</p>
        )}
      </div>
    </section>
  );
