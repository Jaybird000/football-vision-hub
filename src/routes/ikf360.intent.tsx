...
function AlreadySubmitted({ intent }: { intent: MyIntent }) {
  ...
  const submittedDate = new Date(intent.submittedAt).toLocaleDateString("en-IN", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-up space-y-8">
      <div className="mb-2">
        <div
          className="flex items-center justify-between text-[11px] uppercase tracking-[0.16em] mb-2"
          style={{ color: "var(--ikf-text-dim)" }}
        >
        ...
        <div
          ...
          style={{ background: "var(--ikf-surface-2)" }}
        >
      ...
      <header className="flex items-start gap-4">
        ...
        <div>
          <h1 className="text-[26px] sm:text-[30px] leading-tight">
            Your child's profile has been started.
          </h1>
      ...
      <div
        className="ikf-card p-5 flex items-start gap-3"
        style={{ background: "var(--ikf-surface-2)" }}
      >
  ...
  return (
    <div className="ikf-card p-7 space-y-5">
      ...
      <div className="pt-5 border-t" style={{ borderColor: "var(--ikf-border)" }}>
        ...
        <p className="mt-3 text-[13px] leading-relaxed" style={{ color: "var(--ikf-text-dim)" }}>
          {STAGE1_EXPLAINER.curiosity}
        </p>
      </div>
    </div>
  );
...
function JourneyForm({
  ...
  // The wizard advances steps within a single route, so the router's scroll
  // restoration never fires. Reset to the top on every step/section change so
  // each new screen opens from the top, not wherever the last button was.
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [step, pos]);
  ...
  async function submit() {
    ...
    try {
      ...
      console.error(err);
      setSubmitError(
        err instanceof Error ? err.message : "Something went wrong saving your responses.",
      );
      setStep("section");
    }
  }
    ...
    return (
      <div className="max-w-2xl mx-auto animate-fade-up">
        ...
        <button
          ...
          className="ikf-btn-primary inline-flex items-center gap-2 mt-7"
        >
    ...
    return (
      <div className="max-w-2xl mx-auto animate-fade-up">
        ...
        <div className="ikf-card p-8 mt-6 text-center">
          <div
            className="w-12 h-12 rounded-full inline-flex items-center justify-center mb-5"
            style={{ background: "var(--ikf-surface-2)", color: "var(--ikf-brand-ink)" }}
          >
        ...
        <div className="flex items-center justify-between mt-7">
          ...
          <button
            ...
            className="ikf-btn-primary inline-flex items-center gap-2"
          >
    ...
    return (
      <div className="max-w-2xl mx-auto animate-fade-up flex flex-col items-center justify-center py-24 text-center">
        <Loader2
          ...
          style={{ color: "var(--ikf-brand-ink)" }}
        />
        <div className="text-[15px]" style={{ color: "var(--ikf-text-dim)" }}>
          Starting your child's profile…
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-2xl mx-auto animate-fade-up space-y-8">
        <div className="text-center">
          ...
          <h1 className="text-[26px] sm:text-[32px] leading-tight">
            {JOURNEY_SUBMISSION.headline}
          </h1>
          <p className="mt-3 text-[15px]" style={{ color: "var(--ikf-text-dim)" }}>
            {JOURNEY_SUBMISSION.intro}
          </p>
        </div>
        ...
        <div className="ikf-card p-7 space-y-4">
          {JOURNEY_SUBMISSION.body.map((p, i) => (
            <p
              ...
              style={
                i === JOURNEY_SUBMISSION.body.length - 1
                  ? undefined
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-up">
      ...
      {reusedFamily && (
        <div
          className="mt-3 mb-1 ikf-card p-3.5 flex items-center justify-between gap-3"
          style={{ background: "var(--ikf-surface-2)" }}
        >
      ...
      <header className="mt-4 mb-7">
        ...
        <p className="mt-3 text-[14px] leading-relaxed" style={{ color: "var(--ikf-text-dim)" }}>
          {sectionMeta.subtext}
        </p>
      </header>
      ...
      <div className="flex items-center justify-between mt-9">
        ...
        <button
          onClick={advance}
          disabled={!valid}
          className="ikf-btn-primary inline-flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
        >
          {!isLast ? (
            <>
            ...
            <>
              Start my child's profile <ArrowRight size={16} />
            </>
          )}
        </button>
      </div>
...
function sectionValid(section: number, r: SopResponses): boolean {
  ...
  if (section === 2) {
    const concernOk =
      r.q6.choices.length >= 1 &&
      (!r.q6.choices.includes("Something else") || Boolean(r.q6.other.trim()));
        ...
        <div className="space-y-3">
          {q8.options.map((o) => (
            <OptionButton
              key={o.label}
              label={o.label}
              selected={r.q8 === o.label}
              onClick={() => set({ q8: o.label })}
            />
          ))}
          ...
          <button
            ...
            style={
              r.q8 === PREFER_NOT_TO_SAY
                ? {
                    borderColor: "var(--ikf-brand)",
                    background: "var(--ikf-surface-2)",
                    color: "var(--ikf-text)",
                  }
                : {
                    borderColor: "var(--ikf-border)",
          ...
          {q("q6").options.map((o) => {
            ...
            return (
              <OptionButton
                ...
                onClick={() => {
                  if (selected)
                    set({ q6: { ...r.q6, choices: r.q6.choices.filter((c) => c !== o.label) } });
          ...
          <div className="mt-4">
            <Field label="Tell us more">
              <input
                className="ikf-input"
                value={r.q6.other}
                onChange={(e) => set({ q6: { ...r.q6, other: e.target.value } })}
...
function SectionThree({ r, set }: SectionProps) {
  return (
    <>
      <QuestionBlock
        title="What is your child's name and age?"
        note="First name is enough — we already have the rest from your account."
      >
        <div className="grid sm:grid-cols-2 gap-5">
          ...
          <Field label="Age">
            ...
            >
              ...
              {Array.from({ length: 11 }, (_, i) => 8 + i).map((a) => (
        ...
        <div className="space-y-3">
          {q("q4").options.map((o) => (
            <OptionButton
              key={o.label}
              label={o.label}
              selected={r.q4.prior === (o.label === "Yes" ? "yes" : "no")}
          ...
          <div className="mt-4">
            <Field label={q("q4").followPrompt ?? "Tell us more"}>
              <input
                className="ikf-input"
                value={r.q4.detail}
                onChange={(e) => set({ q4: { ...r.q4, detail: e.target.value } })}
                placeholder="e.g. Pune, around March 2025"
              />
            </Field>
          </div>
        )}
...
function SectionFour({ r, set }: SectionProps) {
  return (
    <>
      ...
      <QuestionBlock
        title="Is there anything else we should know about this child?"
        note="Optional — an open space for anything that doesn't fit a dropdown."
      >
        <textarea
    ...
    <div className="mb-2">
      ...
      >
        <div
          ...
          style={{ width: `${(step / total) * 100}%`, background: "var(--ikf-brand)" }}
        />
      </div>
    </div>
  );
...
function QuestionBlock({
  ...
  return (
    <div>
      ...
      {note && (
        <p className="text-[12px] mb-3" style={{ color: "var(--ikf-text-dim)" }}>
          {note}
        </p>
      )}
...
function TrustChip({ icon: Icon, text }: { icon: typeof Clock; text: string }) {
  return (
    ...
    >
      ...
      <span className="text-[12px] leading-tight" style={{ color: "var(--ikf-text-dim)" }}>
        {text}
      </span>
    </div>
  );
